// app/my-page/page.tsx '내 일정' 목록 페이지로, 서버에서 사용자의 전체 여행 계획 데이터를 불러오는 역할

import { createServerClient } from '@/lib/supabaseClient';
import Link from 'next/link';
import PlanList from '@/component/my-page/PlanList'; // 클라이언트 컴포넌트인 PlanList를 import 합니다.
    ...
    return (
        <div className="p-8 max-w-6xl mx-auto">
            <header className="flex justify-between items-center mb-6">
                ...
                <Link href="/planner/new" className="px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600">
                    + 새 일정 만들기
                </Link>
            </header>
            ...
            <div className="flex items-center gap-4 mb-4">
                ...
                <Link href="/my-page?sort=desc" className={`px-3 py-1 rounded-full text-sm ${!isAscending ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>
                    최신순
                </Link>
                <Link href="/my-page?sort=asc" className={`px-3 py-1 rounded-full text-sm ${isAscending ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>
                    오래된순
                </Link>
            </div>
