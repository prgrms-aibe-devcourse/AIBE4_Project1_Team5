...
// PlannerEditor와 데이터를 주고받을 때 사용할 '장소' 데이터의 타입을 정의합니다.
...
// 이 페이지 컴포넌트가 URL의 searchParams를 통해 어떤 값들을 받을 수 있는지 정의합니다.
...
export default async function EditPlannerPage({ searchParams }: EditPlannerPageProps) {
    ...
    // 최종적으로 PlannerEditor에 전달될 장소 목록을 담을 빈 배열입니다.
    let places: Place[] = [];
    // '여행지 둘러보기' 목록을 필터링할 기준이 되는 지역 이름들을 담을 배열입니다.
    ...
    try {
        if (trip_id) {
            ...
            // 1. trip_id로 'trip_plan_detail' 테이블에서 해당 일정에 속한 모든 장소들의 ID를 가져옵니다.
            ...
            // 2. 만약 저장된 장소가 있다면, 해당 장소들이 어떤 '지역'에 속하는지 조회합니다.
            if (detailData && detailData.length > 0) {
                ...
                // 3. 중복을 제거한 순수한 지역 이름 목록을 만들어 필터링 기준으로 삼습니다.
                ...
                // 저장된 장소가 없으면 regionNamesForFiltering는 빈 배열이 되고, 아래 조회 로직에서 모든 장소를 가져옵니다.
            }

        } else {
            // [생성 경로]
            // URL 파라미터에서 지역 정보를 직접 추출합니다.
        ...
        // .map()을 사용해 우리가 정의한 Place 타입에 맞는 순수한 객체 배열로 변환합니다.
    ...
    // 최종적으로 가공된 장소 목록을 PlannerEditor 컴포넌트에 props로 전달합니다.
