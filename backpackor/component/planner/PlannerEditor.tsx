        ...
        try {
            ...
            if (tripIdToEdit) {
                ...
                const newPlanDetails = Object.entries(plan).flatMap(([day, places]) =>
                    places.map((p, i) => ({ trip_id: Number(tripIdToEdit), place_id: p.place_id, day_number: parseInt(day), visit_order: i + 1 }))
                );

                if (newPlanDetails.length > 0) {
                    const { error: detailError } = await supabase.from('trip_plan_detail').insert(newPlanDetails);
                ...
                const { data: insertedPlan, error: insertError } = await supabase
                    .from('trip_plan')
                    .insert({
                        // 로그인한 사용자의 ID (user.id)를 사용
                        user_id: user.id,
                        trip_title: tripTitle,
                        trip_start_date: startDateStr,
